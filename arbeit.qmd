...
```{r}
# Multinomial-Modelle neu schätzen ---------------------------------------
...
# Model 2 – Motivation + sozioökonomische Ressourcen
...
# Model 3 – + Integration & Kommunikation
...
# coefficients to keep and their order
vars <- c("^interest_z$", "^trust_parliament_z$", "^trust_council_z$",
          "^edu_grouphigh$", "^income_cathigh$",
          "^job_catparttime$", "^job_catother$")

stargazer(
  model1, model2_clean,
  type   = "latex",
  title  = "Multinomial logit models of persistent non-voting (reference = voters)",
  label  = "tab:multinom_results",

  # ---- header ------------------------------------------------------
  column.labels   = rep(c("Apathetic", "Protest", "Satisfied"), 2),
  column.separate = c(3, 3),      # first 3 cols = Model 1, next 3 = Model 2
  model.names     = FALSE,
  dep.var.caption = "",
